...
import { index } from "@/api/system/action";
import { index as getMenus } from "@/api/system/menu";
import { index as getActionGroups } from "@/api/system/action-group";
export default {
        ...
        async getOption() {
            try {
                const results = await Promise.all([
                    getMenus({ per_page: 9999 }),
                    getActionGroups({ per_page: 9999 })
                ]);
                [this.options.menus, this.options.groups] = results.map(
                    item => item.data
                );
            } catch (error) {
                console.log(error);
