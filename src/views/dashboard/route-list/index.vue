...
import { getRouteLists as index } from "@/api/business/work";
import { index as getWarehouses } from "@/api/business/warehouse";
export default {
    ...
    methods: {
        ...
        async getOption() {
            try {
                const results = await Promise.all([getWarehouses()]);
                [this.options.warehouses] = results.map(item => item.data);
            } catch (error) {
                console.log(error);
            }
        },
