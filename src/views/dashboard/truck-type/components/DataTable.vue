    ...
    >
        <template v-slot:top>
            <v-toolbar class="custom-toolbar" flat>
                <v-toolbar-title>Danh sách hãng xe</v-toolbar-title>
        ...
        <template v-slot:item.actions="{ item }">
            <!-- <v-icon small class="mr-2" @click="$emit('handle-edit', item)"
                >mdi-pencil</v-icon
            >-->
            <v-btn x-small @click="$emit('handle-edit', item)" fab dark color="primary">
                <v-icon dark>mdi-pencil</v-icon>
            </v-btn>
            <v-btn x-small @click="handleDelete(item.id)" class="ml-2" fab dark color="error">
...
<script>
import { destroy } from "@/api/business/truck-manufacturer";
...
export default {
    ...
    data() {
        return {
            headers: [
                { text: "Tên hãng xe", value: "name" },
                { text: "Mã", value: "code" },
                { text: "Trọng tải (tấn)", value: "tonnage", align: "right" },
